...
ManagedStatic<std::vector<LiteralOption>> LiteralOptions;
ManagedStatic<std::set<OptionCategory *>> OptionCategories;
ManagedStatic<std::set<Option *>> Options;
ManagedStatic<std::set<SubCommand *>> SubCommands;

std::vector<LiteralOption> &LiteralOptions_Ref = *LiteralOptions;
std::set<OptionCategory *> &OptionCategories_Ref = *OptionCategories;
std::set<Option *> &Options_Ref = *Options;
std::set<SubCommand *> &SubCommands_Ref = *SubCommands;
...
public:
  ...
  CommandLineParser() {
    ...
    for (OptionCategory *optionCategory : *OptionCategories) {
      registerCategory(optionCategory);
    }
    ...
    for (LiteralOption &literalOption : *LiteralOptions) {
      this->addLiteralOption(literalOption.O, literalOption.Name);
    }
...
thread_local static ManagedStatic<CommandLineParser> GlobalParser;

void cl::AddLiteralOption(Option &O, StringRef Name) {
  if (GlobalParser.isConstructed()) {
    GlobalParser->addLiteralOption(O, Name);
  }
  else {
    auto &LiteralOption = LiteralOptions->emplace_back(O, std::string(Name));
  }
}
...
thread_local ManagedStatic<SubCommand> llvm::cl::TopLevelSubCommand;

// A special subcommand that can be used to put an option into all subcommands.
thread_local ManagedStatic<SubCommand> llvm::cl::AllSubCommands;
...
void SubCommand::registerSubCommand() {
  SubCommands->insert(this);
  // GlobalParser->registerSubCommand(this);
}

void SubCommand::unregisterSubCommand() {
  SubCommands->erase(this);
...
thread_local static ManagedStatic<CommandLineCommonOptions> CommonOptions;
